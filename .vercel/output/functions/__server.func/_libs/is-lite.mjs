//#region node_modules/is-lite/dist/index.mjs
var objectTypes = [
	"Array",
	"ArrayBuffer",
	"AsyncFunction",
...
var isArrayOf = (target, predicate) => {
	if (!isArray(target) || !isFunction(predicate)) return false;
...
var isInstanceOf = (instance, class_) => {
	if (!instance || !class_) return false;
...
var isPropertyOf = (target, key, predicate) => {
	if (!isObject(target) || !key) return false;
...
function is(value) {
	...
	if (isPlainFunction(value)) return "Function";
