//#region node_modules/@gilbarbara/deep-equal/dist/index.mjs
...
function compareObjects(left, right, seen) {
	if (hasSeen(seen, left, right)) return true;
	markSeen(seen, left, right);
	if (left.constructor !== right.constructor) return false;
	if (Array.isArray(left) && Array.isArray(right)) return equalArray(left, right, seen);
	if (left instanceof Map && right instanceof Map) return equalMap(left, right, seen);
	if (left instanceof Set && right instanceof Set) return equalSet(left, right);
	if (left instanceof WeakMap || left instanceof WeakSet) return false;
	...
	if (left instanceof Error && right instanceof Error) return equalError(left, right, seen);
...
function compareValues(left, right, seen) {
	...
	if (!left || !isObject(left) || !right || !isObject(right)) return false;
	return compareObjects(left, right, seen);
}
function equalArray(left, right, seen) {
	const { length } = left;
	if (length !== right.length) return false;
	for (let index = length; index-- !== 0;) if (!compareValues(left[index], right[index], seen)) return false;
	return true;
}
function equalArrayBuffer(left, right) {
	...
	while (index--) if (view1.getUint8(index) !== view2.getUint8(index)) return false;
	return true;
}
...
function equalMap(left, right, seen) {
	...
	for (const entry of left.entries()) if (!right.has(entry[0])) return false;
	for (const entry of left.entries()) if (!compareValues(entry[1], right.get(entry[0]), seen)) return false;
	return true;
}
function equalPlainObject(left, right, seen) {
	const leftKeys = Object.keys(left);
	if (leftKeys.length !== Object.keys(right).length) return false;
	for (let index = leftKeys.length; index-- !== 0;) if (!Object.prototype.hasOwnProperty.call(right, leftKeys[index])) return false;
	for (let index = leftKeys.length; index-- !== 0;) {
		const key = leftKeys[index];
		if (key === "_owner" && left.$$typeof) continue;
		if (!compareValues(left[key], right[key], seen)) return false;
	}
	return true;
}
